    ...
    <div className="relative min-h-screen w-full flex flex-col flex-1">
      ...
      <div className={`relative z-10 flex-1 flex flex-col transition-opacity duration-1000 ${isZenMode ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        ...
        <section className="relative z-10 flex min-h-[100svh] flex-col items-center justify-center px-5 pb-28 pt-32 text-center sm:px-6 md:min-h-screen md:pb-24 md:pt-36">
          <h1
            className="hero-title animate-fade-rise max-w-[12ch] text-[2.9rem] font-semibold leading-[1.05] text-foreground sm:max-w-4xl sm:text-7xl md:max-w-7xl md:text-8xl"
          ...
          <p className="hero-copy animate-fade-rise-delay mt-6 max-w-xl text-sm leading-relaxed text-white/[0.78] sm:mt-8 sm:text-base md:text-lg">
      ...
      <section id="featured-journal" className="relative z-10 w-full overflow-hidden px-4 py-24 pb-36 sm:px-6 md:py-32">
        <div className="home-reading-band absolute inset-0" />
        <div className="relative max-w-7xl mx-auto">
        <div className="mb-10 flex flex-col justify-between gap-6 md:mb-16 md:flex-row md:items-end md:gap-8">
          <div>
            <h2 className="section-heading mb-4 text-3xl font-semibold text-foreground sm:text-4xl md:text-6xl" style={{ fontFamily: "'Noto Serif Display', serif" }}>
              Featured Today.
            </h2>
            <p className="section-copy max-w-xl text-base leading-relaxed text-white/[0.76] md:text-lg">
              One tech signal, one life signal, one debate signal. A calmer way to enter the archive.
            </p>
          </div>
          <Link to="/journal" className="group flex items-center gap-2 text-white/[0.72] transition-colors hover:text-foreground">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-3 md:gap-8">
          ...
          {!isLoading && featuredArticles.map((entry, idx) => (
            <article 
              ...
              className="featured-entry group flex min-h-[220px] cursor-pointer flex-col border-t border-white/18 py-6 transition-colors animate-[fade-rise_0.6s_ease-out] hover:border-white/45 md:min-h-[300px] md:py-8"
              style={{ animationDelay: `${idx * 0.15}s`, animationFillMode: 'both' }}
              onClick={() => {
                navigate(`/journal/${encodeURIComponent(entry.article.id)}`);
              }}
            >
              <span className="mb-5 font-mono text-[11px] uppercase tracking-[0.24em] text-white/[0.72]">
                {entry.label}
              </span>
              <div className="mb-6 flex flex-wrap items-center gap-4 font-mono text-xs text-white/[0.68]">
              ...
              <p className="mb-8 line-clamp-3 text-sm leading-relaxed text-white/[0.76]">
                {entry.article.excerpt}
              </p>

              <div className="flex flex-wrap gap-2 mt-auto">
                {entry.article.tags.map(tag => (
                  <span key={tag} className="font-mono text-[10px] uppercase tracking-[0.18em] text-white/[0.66]">
                    {displayTagLabel(tag)}
                  </span>
                ))}
              </div>
        ...
        {!isLoading && articles.length > 0 && (
          <div className="featured-entry mt-12 flex w-fit max-w-full flex-wrap items-center justify-center gap-x-4 gap-y-2 border-t border-white/18 pt-4 text-sm text-white/[0.72] md:mt-16">
